...
If the argument is `next`, run the same routing as `/agtoosa-next dry` per `Docs/AgToosa_Next.md`. End with `To execute: /agtoosa-next`. Do not run mutating commands from help. **Do not** use for freeform `next` — PROGRESS utterances execute `/agtoosa-next`.
