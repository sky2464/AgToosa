...
15. **Story Skill Opportunity Synthesis (Codex / OpenCode):**
    ...
    *   **Reserved workflow names:** reject candidates named `agtoosa-*`, triggered by `/agtoosa-*`, or that would shadow `.cursor/commands/agtoosa-*.md` or `.windsurf/workflows/agtoosa-*.md` unless updating an installed AgToosa workflow adapter — those names are reserved for AgToosa lifecycle commands, not generated project skills.
