...
- **Reserved workflow names:** `agtoosa-*` names and `/agtoosa-*` triggers are owned by installed AgToosa workflow adapters (`.codex/skills/agtoosa-*`, `.github/prompts/agtoosa-*.prompt.md`, and platform equivalents). Generated project skills must not use `agtoosa-*` names or `/agtoosa-*` triggers unless **Update existing** on an AgToosa adapter — otherwise choose **Do not generate**.
