...
1. Print a **primary lifecycle next-step** line — **not** `/agtoosa-status` as the headline:
    ...
    For sequential users, `/agtoosa-next` is the default handoff (it reads SYNC and dispatches the underlying phase). Advanced users may still invoke `/agtoosa-build`, `/agtoosa-review`, etc. directly.
