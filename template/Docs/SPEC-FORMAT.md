...
The scope boundary block generated by `/agtoosa-spec` Part 4. Copy it verbatim into this section.
...
Every Must-priority AC must appear in the test plan. Should and Could ACs should appear where practical.
...
This marker is required by `/agtoosa-ship check`. Do not add it manually — let `/agtoosa-spec` append it after human review. Do not remove it once added.
