...
3. **Dispatch `next`:** same routing as `/agtoosa-next dry` per `Docs/AgToosa_Next.md`; end with `To execute: /agtoosa-next` — do not auto-run mutating workflows. **Only** when user explicitly invokes `/agtoosa-help next` — freeform `next` executes `/agtoosa-next`.
