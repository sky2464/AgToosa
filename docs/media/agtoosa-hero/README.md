...
| `ReadmeLoop` | 32s | Readable cinematic README motion | Silent |
...
- The README loop uses restrained Manrope for human language and IBM Plex Mono
  only for commands, uppercase technical labels, and authentic verifier output.
...
- The silent README loop gives status, init, animated workflow, routing,
  verification, a captioned full-workflow overview, and closing-brand scenes
  deliberate reading holds. It exports all 960 frames at 30fps with compact
  inline typography and an 800×450 source for smooth, legible inline playback
  without a separate media link.
