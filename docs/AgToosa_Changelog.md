...
- **Pipe bootstrap project path prompt.** `curl | bash` install no longer exits before the user can enter a project path; prompts read from `/dev/tty` when stdin is the script stream. Empty path defaults to `.` with clearer Enter/current-folder guidance (DEV-149 B33-001–B33-005; PR #100).
...
- **DEV-144 — Gitignore auto-merge.** Marker-bounded `.gitignore` block on install/update; doctor GIG-003/GIG-004; operational-only scope (`.agtoosa/`, `.worktrees/`, `*.bak.*`); GIG-001–GIG-008 bats.
...
- **DEV-143 — Tracker status-check.** `--tracker status-check` CLI; `.agtoosa/tracker/` cache auto-merge; `AgToosa_Status.md` Info finding + Part 5.5 row; TUS-001–TUS-008 bats.
...
- **DEV-142 — GitHub surface audit.** Manifest + local/live audit scripts; GITHUB-SURFACES runbook; issue template config; labels sync from manifest; CI surface-audit workflow; GSA-001–GSA-010 bats.
...
- **DEV-140 — Help vs Next disambiguation.** PROGRESS vs `/agtoosa-help next` table; forbidden closure anti-patterns; adapter hardening; `progress-continuation-proof` BCL scenario; NLX-009–013 bats; ADR-019 amended.
...
- **DEV-133 — Branch hygiene.** Maintainer `scripts/cleanup-github-branches.sh` (dry-run default, `cursor/` prefix filter, main/master denylist); weekly `.github/workflows/branch-hygiene.yml`; runbook in `docs/agtoosa-maintainer.md`; BRH-001–006 bats.
...
- 2026-07-08 — fix — v0.3.1 patch: pack lock provenance, re-install data loss, multi-root pack smuggle, npm/bootstrap durable pack queue (`AGTOOSA_PACK_QUEUE_DIR`), PowerShell hooks deep-merge and merge containment — `CHANGELOG.md`
