...
>
> **Maintainer mirror:** `docs/AgToosa_Network_Matrix.md`. Generated projects install `Docs/AgToosa_Network_Matrix.md` from `template/Docs/AgToosa_Network_Matrix.md`.
...
| verify | `bash agtoosa.sh --verify [path]` | `.\agtoosa.ps1 -Verify -UpdatePath <path>` | offline | Runs the local lifecycle verifier (`Docs/agtoosa-verify.sh` or `docs/agtoosa-verify.sh`). |
...
- Maintainer guide: `docs/agtoosa-maintainer.md`

<!-- AGTOOSA PRODUCT TRUTH START: claims.surface.network -->
...
<!-- AGTOOSA PRODUCT TRUTH END: claims.surface.network -->
