...
DEV-058 captures one candidate capability from the competitive execution wave. It is a backlog spec until explicitly enrolled and built.

## Goal Contract

| Field | Value |
|-------|-------|
| Goal | Provide a local static or TUI dashboard for Master-Plan, tasks, evidence, blockers, and next actions. |
| User outcome | Users can inspect AgToosa state without manually reading several markdown files. |
| Success condition | Dashboard spec defines read-only rendering, data sources, and no-mutation guarantees. |
| Proof / evidence | Dashboard docs, read-only tests, focused bats checks, and test-plan evidence. |
...
| Non-goals | This story does not create a hosted project-management service. |
...
| ID | Requirement |
|----|-------------|
| AC-001 | WHEN DEV-058 is read THE SYSTEM SHALL state the specific user outcome and proof required before the capability is treated as shipped. |
