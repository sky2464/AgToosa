...
DEV-055 captures one candidate capability from the competitive execution wave. It is a backlog spec until explicitly enrolled and built.

## Goal Contract

| Field | Value |
|-------|-------|
| Goal | Detect installed agent surfaces and choose the best handoff, review, or build path per story. |
| User outcome | AgToosa can route work to available tools without assuming every platform has every capability. |
| Success condition | Capability matrix maps surfaces to commands, handoff support, review support, and fallbacks. |
| Proof / evidence | Matrix docs, adapter references, focused bats checks, and test-plan evidence. |
...
| Non-goals | This story does not claim native support where platforms only have fallback docs. |
...
| ID | Requirement |
|----|-------------|
| AC-001 | WHEN DEV-055 is read THE SYSTEM SHALL state the specific user outcome and proof required before the capability is treated as shipped. |
...
Test plan: `docs/AgToosa_TestPlan-DEV-055.md`
