...
DEV-046 captures one candidate capability from the competitive execution wave. It is a backlog spec until explicitly enrolled and built.

## Goal Contract

| Field | Value |
|-------|-------|
| Goal | Add guidance and checks for optional per-lane worktree isolation on larger stories. |
| User outcome | Maintainers can isolate risky lanes without forcing worktrees on every small change. |
| Success condition | Workflow docs explain when to create worktrees, how to keep them ignored, and how to integrate results. |
| Proof / evidence | Workflow docs, safety checklist, focused bats checks, and test-plan evidence. |
...
| Non-goals | This story does not require hosted branch orchestration. |
...
| ID | Requirement |
|----|-------------|
| AC-001 | WHEN DEV-046 is read THE SYSTEM SHALL state the specific user outcome and proof required before the capability is treated as shipped. |
