...
DEV-059 captures one candidate capability from the competitive execution wave. It is a backlog spec until explicitly enrolled and built.

## Goal Contract

| Field | Value |
|-------|-------|
| Goal | Define allowed tools, paths, network, secrets, approval levels, and risky actions as repo-local policy. |
| User outcome | Teams can make agent boundaries explicit before work starts. |
| Success condition | Policy schema documents enforcement class for each rule and workflow behavior on violations. |
| Proof / evidence | Policy docs, examples, focused bats checks, and test-plan evidence. |
...
| Non-goals | This story does not claim enforcement beyond wired generator/CI/agent-instructed controls. |
...
| ID | Requirement |
|----|-------------|
| AC-001 | WHEN DEV-059 is read THE SYSTEM SHALL state the specific user outcome and proof required before the capability is treated as shipped. |
