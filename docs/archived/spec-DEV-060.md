# Spec: DEV-060 - Public Benchmark Suite
...
DEV-060 captures one candidate capability from the competitive execution wave. It is a backlog spec until explicitly enrolled and built.

## Goal Contract

| Field | Value |
|-------|-------|
| Goal | Create repeatable AgToosa challenge tasks comparing spec-to-test-to-ship outcomes against alternatives. |
| User outcome | Prospective users can evaluate AgToosa with reproducible tasks and evidence. |
| Success condition | Benchmark suite defines tasks, scoring, evidence requirements, and honest limitations. |
| Proof / evidence | Benchmark docs, README link, focused bats checks, and test-plan evidence. |
...
| Non-goals | This story does not claim superiority without published benchmark evidence. |
...
| ID | Requirement |
|----|-------------|
| AC-001 | WHEN DEV-060 is read THE SYSTEM SHALL state the specific user outcome and proof required before the capability is treated as shipped. |
...
Test plan: `docs/AgToosa_TestPlan-DEV-060.md`
