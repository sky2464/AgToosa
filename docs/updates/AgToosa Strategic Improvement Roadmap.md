...
> **Snapshot:** 2026-07-11, after v5.3.6; DEV-055 is the active cycle.
...
- **Now:** DEV-055, Agent Capability Matrix, is enrolled and awaiting explicit spec approval for the v5.3.7 target.
...
| Repo-local policy and optional hooks | Strengthens boundaries without a hosted control plane | Deepen DEV-059/052 specs |
| Verifier and CI adoption | Already shipped; needs usage evidence and documentation | Publish benchmark and gate examples |
| High-quality registry presets | Uses the current registry instead of inventing a plugin runtime | Validate three maintained packs before scaling |
| Read-only local visibility | Preserves Master-Plan authority | DEV-058 after evidence sources stabilize |
| Documentation and onboarding | Reduces adoption friction without runtime complexity | Static docs proof, guides, current first-15-minute path |

### 5.2 Validate first

| Opportunity | Evidence required before enrollment |
|---|---|
| Thin native wrapper | Repeated installation failures that Bash/PowerShell parity cannot solve |
| Editor extension | Repeated user demand for command discovery or Master-Plan navigation |
| Tracker bridge | Teams requesting one-way export/import while accepting Master-Plan as source of truth |
| Fail-closed signatures | Users willing to manage key rotation and reject unsigned packs/releases |
| Multi-repo overlay | At least one real multi-repo story that cannot be handled with separate handoff packs |
| Official pack expansion | Successful use and maintenance of the first three packs |
| Lightweight paid support | Sustained support demand and a service level the maintainer can actually meet |
...
- Add a subagent walkthrough: spec → two handoff lanes → import → review.

### 6.2 Documentation

A static docs site is a distribution surface, not a new runtime. Validate it with a minimal GitHub Pages build from existing markdown. Do not add accounts, a backend, or a second source of documentation truth.

Priority guide topics:

1. AgToosa for subagent-heavy workflows.
2. AgToosa for security-sensitive projects.
3. AgToosa for solo developers.
4. Writing and verifying registry packs.

### 6.3 Registry

Start with three maintained packs, not a marketplace:
...
Low-complexity options can proceed independently of product architecture:
...
AgToosa does not need telemetry to collect initial evidence. Public repositories, voluntary surveys, support issues, benchmark runs, and case studies are sufficient until users explicitly request more.
...
Finish DEV-055 before enrolling another implementation story. Then deepen DEV-045 as the next subagent-safety contract. In parallel, close the high-leverage manual distribution and benchmark tasks already recorded in Master-Plan.
