...
OBJECT_KEYS = {
    ...
    "command": {"id", "phase", "invocation", "modes", "canonical_workflow",
                ...
                "portable_invariants", "host_mode_policy"},
    "question_budget": {"max_questions", "quick_max"},
    "host_mode_policy": {"auto_switch", "plan_phases", "agent_phases"},
...
def exact_fields(obj, expected: set[str], where: str, findings: Findings,
    ...
    for key in sorted(expected - set(obj) - optional):
...
def validate_schema(data: dict, root: Path, findings: Findings) -> None:
    ...
    for index, item in enumerate(data.get("commands", [])):
        exact_fields(item, OBJECT_KEYS["command"], f"commands[{index}]", findings,
                     optional={"host_mode_policy"})
        exact_fields(item.get("question_budget"), OBJECT_KEYS["question_budget"],
                     f"commands[{index}].question_budget", findings)
        if "host_mode_policy" in item and item["host_mode_policy"] is not None:
            exact_fields(item.get("host_mode_policy"), OBJECT_KEYS["host_mode_policy"],
                         f"commands[{index}].host_mode_policy", findings)
        elif "host_mode_policy" in item:
            findings.add(f"commands[{index}].host_mode_policy: must be object when present")
...
def managed_block(command: dict) -> str:
    ...
    policy = command.get("host_mode_policy")
    if policy:
        plan_phases = ", ".join(f"`{phase}`" for phase in policy["plan_phases"])
        agent_phases = ", ".join(f"`{phase}`" for phase in policy["agent_phases"])
        lines += [
            f"- Host mode: auto-switch {str(policy['auto_switch']).lower()}",
            f"- Plan phases: {plan_phases}",
            f"- Agent phases: {agent_phases}",
        ]
    lines.append(end)
    return "\n".join(lines)
