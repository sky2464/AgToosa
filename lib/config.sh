...
agtoosa_project_owned_doc_preserve_reason() {
  local f="$1"
  case "$f" in
    Docs/Master-Plan.md) echo "your project plan" ;;
    Docs/AgToosa_Changelog.md) echo "your changelog" ;;
    Docs/Master-Architecture.md) echo "your architecture" ;;
    Docs/agtoosa-evidence.jsonl) echo "your evidence ledger" ;;
    *) echo "your project state" ;;
  esac
}
...
print_usage() {
  ...
  echo "  --tracker <cmd>        Tracker Sync Bridge (export · propose · publish · intake · discover · bootstrap)"
  echo "                           export            — Export Master-Plan stories to neutral JSON"
  echo "                           propose           — Validate return envelope; write proposal artifact"
  echo "                           publish           — Render GitHub Issues manifest (+ optional --readme)"
  echo "                           intake            — Community issue → proposal + backlog draft"
  echo "                           discover          — Scan repo for tracker signals and external items"
  echo "                           bootstrap         — Classify discovery into proposal (--apply to merge)"
  echo "                           status-check      — Report unlinked external items (DEV-143)"
  echo "  --apply                With bootstrap: merge proposal JSON into Master-Plan (dry-run default)"
  echo "  --apply-all-new-external  With bootstrap --apply: auto-accept all new_external rows"
  echo "  --path <dir>           Project path (install wizard, or --tracker target)"
  echo "  --input <file>         Return, intake, discovery merge, or bootstrap input (with --tracker)"
  echo "  --output <file>        Export, manifest, or proposal output path (with --tracker)"
  echo "  --readme <file>        README for AGTOOSA-ROADMAP block (with --tracker publish)"
  echo "  --update [path]        Update an existing AgToosa install (default safe upgrade)"
  echo "  --reinstall --clean    Optional destructive fresh reinstall (ADR-004 Option C)"
  echo "  --accept-breaking      Allow MAJOR-version migration apply (prints plan first)"
  ...
  echo "  --transaction-recover [path]  Restore project tree from newest incomplete transaction journal"
  echo "  --transaction-status [path]   List transaction journals for a project"
  echo "  --transaction-id <id>  With --transaction-recover: target a specific journal"
  ...
  echo "  --verbose              With --cleanup: list every candidate file (default: compact summary)"
