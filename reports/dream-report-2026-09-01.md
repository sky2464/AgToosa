...
2. **#146 (CI `validate` job failing on `main`) — re-verified today, not re-litigated.** Actually ran `bats tests/agtoosa.bats` this time (installed via `apt-get install bats` — confirms #146's suspicion that this routine's sandbox not having `bats` preinstalled was masking this). Partial results (~976/1336 tests, 94 failures) already track closely with #146's ~125/1336 total; a comment with the final count follows once the full run completes. Still assessed as too large/design-dependent for a single automated fix, consistent with the prior routine's own judgment — left open rather than reopening the design question.
...
3. **(P3, tracked, no automated fix)** #146 remains open pending human triage of the ~125-test CI failure; #141 remains open pending human Dependabot triage. Both re-confirmed today via direct verification rather than left stale.
